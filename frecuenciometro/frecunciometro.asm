...
;frecunciometro.c,23 :: 		INTCON.GIE = 0;        // Se deshabilitan todas las interrupciones
	BCF        INTCON+0, 7
;frecunciometro.c,24 :: 		if (INTCON.T0IF == 1 ) // se chequea si el timer 0 fue el causante
	BTFSS      INTCON+0, 2
	GOTO       L_interrupt0
;frecunciometro.c,26 :: 		INTCON.T0IF = 0;   //se baja la bandera del timer0
	BCF        INTCON+0, 2
...
;frecunciometro.c,44 :: 		TMR1L  = 0x00;        //registro de los segiundos 8 bits del timer1
	CLRF       TMR1L+0
;frecunciometro.c,46 :: 		Lcd_Init();                        // Initialize LCD
	CALL       _Lcd_Init+0
;frecunciometro.c,47 :: 		Lcd_Cmd(_LCD_CLEAR);               // Clear display
	MOVLW      1
	MOVWF      FARG_Lcd_Cmd_out_char+0
	CALL       _Lcd_Cmd+0
;frecunciometro.c,48 :: 		Lcd_Cmd(_LCD_CURSOR_OFF);          // Cursor off
	MOVLW      12
	MOVWF      FARG_Lcd_Cmd_out_char+0
	CALL       _Lcd_Cmd+0
;frecunciometro.c,49 :: 		Lcd_Out(2,5,"FRECUENCIA:");        // Write text in second row
	MOVLW      2
	...
	CALL       _Lcd_Out+0
;frecunciometro.c,51 :: 		while(1)
L_main1:
;frecunciometro.c,60 :: 		if (ctrl==123)
	MOVF       _ctrl+0, 0
	XORLW      123
	BTFSS      STATUS+0, 2
	GOTO       L_main3
;frecunciometro.c,62 :: 		printf=1;
	MOVLW      1
	MOVWF      _printf+0
;frecunciometro.c,63 :: 		}
L_main3:
;frecunciometro.c,64 :: 		if (printf==1)
	MOVF       _printf+0, 0
	XORLW      1
	BTFSS      STATUS+0, 2
	GOTO       L_main4
;frecunciometro.c,66 :: 		IntToStr(TMR1L, text); //imprimimos el valor acumulado en TIM1L
	MOVF       TMR1L+0, 0
	...
	CALL       _IntToStr+0
;frecunciometro.c,67 :: 		Lcd_Out(3,5,text);
	MOVLW      3
	MOVWF      FARG_Lcd_Out_row+0
	MOVLW      5
	MOVWF      FARG_Lcd_Out_column+0
	...
	MOVWF      FARG_Lcd_Out_text+0
	CALL       _Lcd_Out+0
;frecunciometro.c,68 :: 		Lcd_Out_CP("HZ");
	MOVLW      72
